...
yield* sequence(0.2,
    ...
    defText().text("A tree is like your family tree. It starts with one root and branches out!", 0.5),
);
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("Here's how a tree looks. The top is the root, and each circle is a node.", 0.5),
);

yield* leftAccent().height(cardHeight, 0.3);
yield* waitFor(3.9800000000000004);
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("A node is like a box that holds data. It can have children nodes!", 0.5),
);

yield* leftAccent().height(cardHeight, 0.3);
yield* waitFor(3.6399999999999997);
...
// Process Flow Scene
const steps = ["To create a node, you need data and link"];
...
yield* title().text("CREATING A NODE", 0.3);
    ...
    yield* sequence(0.1,
        ...
        stepTexts[0].text("To create a node, you need data and link", 0.3),
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("Here's how a node is created step by step.", 0.5),
);
...
const pointBgs = createRefArray<Rect>();
    ...
    yield* sequence(0.1,
        ...
        pointTexts[1].text("Nodes hold data and link to children", 0.3),
