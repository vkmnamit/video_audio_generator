...
if ("Imagine finding your favorite toy in a huge pile. How do you do it super fast?") {
    yield* subtitle().text("Imagine finding your favorite toy in a huge pile. How do you do it super fast?", 0.4);
...
yield* sequence(0.2,
    ...
    defText().text("It's like a super-smart guessing game. You cut the pile in half every time!", 0.5),
);
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("Instead of checking every toy one by one, you find it in just a few guesses!", 0.5),
);
...
yield* waitFor(3.0199999999999996);

// Scene 5: process

// Process Flow Scene
const steps = ["First, check the middle", "Too low", "Try the middle of the right half"];
...
if ("First, check the middle. Too low? Try the middle of the right half!") {
    yield* subtitle().text("First, check the middle. Too low? Try the middle of the right half!", 0.4);
    yield* subtitle().opacity(1, 0.3);
}
...
view.add(
    ...
    yield* sequence(0.1,
        ...
        stepTexts[2].text("Try the middle of the right half", 0.3),
...
yield* waitFor(2.9000000000000004);
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("Like when you open a dictionary and guess where 'Apple' is. You don't start at A!", 0.5),
);
...
// Summary Scene
yield* title().text("✅ YOU GOT THIS!", 0.4);
...
const pointBgs = createRefArray<Rect>();
    ...
    yield* sequence(0.1,
        ...
        pointTexts[0].text("Split in half", 0.3),
    ...
    yield* sequence(0.1,
        ...
        pointTexts[1].text("Guess the middle", 0.3),
