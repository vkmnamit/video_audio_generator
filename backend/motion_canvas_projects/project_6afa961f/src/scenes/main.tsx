...
if ("Ever tried finding your favorite song in a huge playlist? What if I told you there's a super-fast way?") {
    yield* subtitle().text("Ever tried finding your favorite song in a huge playlist? What if I told you the", 0.4);
...
yield* sequence(0.2,
    ...
    defText().text("It's like a super-smart guessing game. Instead of checking every item, you cut the list in half each time!", 0.5),
);
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("Imagine finding a word in a dictionary. You don't start from A, you open it in the middle and guess!", 0.5),
);
...
yield* waitFor(2.9299999999999997);

// Scene 5: process

// Process Flow Scene
const steps = ["First, check the middle", "Too low", "Go right", "Too high"];
...
if ("First, check the middle. Too low? Go right. Too high? Go left. Repeat!") {
    yield* subtitle().text("First, check the middle. Too low? Go right. Too high? Go left. Repeat!", 0.4);
    yield* subtitle().opacity(1, 0.3);
}
...
view.add(
    ...
    yield* sequence(0.1,
        ...
        stepTexts[0].text("First, check the middle", 0.3),
...
yield* waitFor(5.2299999999999995);
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("Like when you're looking for a page in a book. You don't go page by page, you flip to the middle!", 0.5),
);
...
const pointBgs = createRefArray<Rect>();
    ...
    yield* sequence(0.1,
        ...
        pointTexts[0].text("Cut the list in half", 0.3),
    ...
    yield* sequence(0.1,
        ...
        pointTexts[1].text("Check the middle", 0.3),
    ...
    yield* sequence(0.1,
        ...
        pointTexts[2].text("Repeat until found", 0.3),
