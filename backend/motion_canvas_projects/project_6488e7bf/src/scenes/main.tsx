...
yield* sequence(0.2,
    ...
    defText().text("It's like a super-smart guessing game. You split stuff in half to find what you want FAST.", 0.5),
);
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("Instead of checking every single thing, you skip half each time. Like magic!", 0.5),
);
...
yield* waitFor(3.5300000000000002);

// Scene 5: process

// Process Flow Scene
const steps = ["First, pick the middle", "Too high", "Go left", "Too low"];
...
if ("First, pick the middle. Too high? Go left. Too low? Go right.") {
    yield* subtitle().text("First, pick the middle. Too high? Go left. Too low? Go right.", 0.4);
    yield* subtitle().opacity(1, 0.3);
}
...
view.add(
    ...
    yield* sequence(0.1,
        ...
        stepTexts[0].text("First, pick the middle", 0.3),
...
yield* waitFor(3.8199999999999994);
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("Like when you open a dictionary. You don't start at page 1, you flip to the middle!", 0.5),
);
