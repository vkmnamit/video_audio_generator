...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("You use queues every day! Printing documents, downloading files, even waiting for your turn in a game.", 0.5),
);
...
// Process Flow Scene
const steps = ["Think of a queue like a lunch line", "New people join at the back, and the first person "];
...
if ("Think of a queue like a lunch line. New people join at the back, and the first person gets") {
    yield* subtitle().text("Think of a queue like a lunch line. New people join at the back, and the first person gets", 0.4);
    yield* subtitle().opacity(1, 0.3);
}
...
view.add(
    ...
    yield* sequence(0.1,
        ...
        stepTexts[0].text("Think of a queue like a lunch line", 0.3),
    ...
    yield* sequence(0.1,
        ...
        stepTexts[1].text("New people join at the back, and the first person ", 0.3),
...
yield* waitFor(3.6500000000000004);
...
yield* sequence(0.2,
    ...
    separator().width(cardWidth - 60, 0.3),
    defText().text("Like when you're waiting for your turn on the playground swing. No cutting, right?", 0.5),
);

yield* leftAccent().height(cardHeight, 0.3);
yield* waitFor(3.8600000000000003);
...
const pointBgs = createRefArray<Rect>();
    ...
    yield* sequence(0.1,
        ...
        pointTexts[2].text("No cutting allowed", 0.3),
        pointBgs[2].opacity(1, 0.2),
    );
    yield* waitFor(0.4);


yield* waitFor(4.6899999999999995);
